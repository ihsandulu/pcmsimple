...
?>
		...
		<script>
			function deleteproductinv(invproduct_id, gudang_id, sjkeluarproduct_id) {
				$.get("<?= site_url("api/deleteproductinv"); ?>", {
						invproduct_id: invproduct_id,
						gudang_id: gudang_id,
	...
	public function insertpaymentinv()
	{
		...
		foreach ($this->input->get() as $e => $f) {
			if ($e != 'create' && $e != 'methodpayment_id' && $e != 'customer_id') {
	...
	public function insertproductinv()
	{
		...
		if ($input["invproduct_qty"] < 0.01) {
			$input["invproduct_qty"] = 0.01;
		}
	...
	?>
		...
		<script>
			...
			function tampilpayment(inv_no, tagihan, methodpayment_id, invpayment_no, customer_id) {
				$('#formproduct').hide();
				$('#listproduct').hide();
				$.get("<?= site_url("api/paymentsimpleinv"); ?>", {
						inv_no: inv_no,
						tagihan: tagihan,
						methodpayment_id: methodpayment_id,
						invpayment_no: invpayment_no,
		...
		?>
			<datalist id="customer">
				...
				foreach ($uni->result() as $customer) { ?>
					<?php if ($identity_project == "2") {
						$ktp = $customer->customer_ktp;
					} else {
						$ktp = "";
					} ?>
	...
	public function perbaikankelurahan()
	{
		$kecamatan = $this->db->get("kecamatan");
		foreach ($kecamatan->result() as $kecamatan) {
			$input["kecamatan_id"] = $kecamatan->kecamatan_id;
			$this->db->where("kecamatan_id", $kecamatan->kecamatan_code)->update("kelurahan", $input);
		}
	}

	function ambilInisial($nama)
	{
		$kata = explode(' ', $nama); // Memecah nama menjadi array berdasarkan spasi
		$inisial = '';

		foreach ($kata as $k) {
			if (!empty($k)) {
				$inisial .= substr($k, 0, 1); // Mengambil karakter pertama dari setiap kata
			}
		}

		return strtoupper(substr($inisial, 0, 3)); // Mengambil 3 karakter pertama dan mengubah ke huruf besar
	}

	public function updatecodekelurahan()
	{
		$kelurahan = $this->db->get("kelurahan");
		foreach ($kelurahan->result() as $kelurahan) {
			$input["kelurahan_code"]=$this->ambilInisial($kelurahan->kelurahan_name);
			$this->db->where("kelurahan_id",$kelurahan->kelurahan_id)->update("kelurahan", $input);
		}
	}

	public function pilihinkota()
	{
	...
	public function pilihinkecamatan()
	{
	...
	public function pilihinkelurahan()
	{
		...
		$kelurahan_id="";
	?>
		<option value="" <?= ($kelurahan_id == "") ? "selected" : ""; ?>>Pilih Kelurahan</option>
		<?php $kelurahan = $this->db
			->where("kecamatan_id", $kecamatan_id)
			->order_by("kelurahan_name", "ASC")
			->get("kelurahan");
			// echo $this->db->last_query();
		foreach ($kelurahan->result() as $kelurahan) { ?>
