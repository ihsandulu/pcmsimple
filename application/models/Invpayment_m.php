...
class invpayment_M extends CI_Model {
	...
	{	
		...
		$path_parts = pathinfo($_FILES['invpayment_picture']['name']);
		$invpayment_picture=str_replace(' ', '_',$path_parts['filename']);
		$invpayment_picture=str_replace('.', '_',$invpayment_picture);
		$invpayment_picture = date("H_i_s_").$invpayment_picture.".".$path_parts['extension'];
		...
		if($this->input->post("create1")=="OK"){
			...
			foreach($this->input->post() as $e=>$f){if($e!='create1'){$input[$e]=$this->input->post($e);}}
		...
		//update
		if($this->input->post("change1")=="OK"){
			foreach($this->input->post() as $e=>$f){if($e!='change1'&&$e!='invpayment_picture'){$input[$e]=$this->input->post($e);}}
