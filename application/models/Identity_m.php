...
class identity_M extends CI_Model
{
	...
	public function data()
	{
		...
		// Upload identity_stempel
		$data['uploadidentity_stempel'] = "";
		if (isset($_FILES['identity_stempel']) && $_FILES['identity_stempel']['name'] != "") {
			$array = explode('.', $_FILES['identity_stempel']['name']);
			$extension = end($array);
			$identity_stempel = str_replace(' ', '_', $_FILES['identity_stempel']['name']);
			$identity_stempel = str_replace('.', '_', $identity_stempel);
			$identity_stempel = $identity_stempel . "." . $extension;
			$identity_stempel = date("H_i_s_") . $identity_stempel;
			if (file_exists('assets/images/identity_stempel/' . $identity_stempel)) {
				unlink('assets/images/identity_stempel/' . $identity_stempel);
			}
			$config['file_name'] = $identity_stempel;
			$config['upload_path'] = 'assets/images/identity_stempel/';
			$config['allowed_types'] = 'gif|jpg|png|jpeg';
			$config['max_size'] = '3000000000';
			$config['max_width'] = '5000000000';
			$config['max_height'] = '3000000000';
			...
			if (!$this->upload->do_upload('identity_stempel')) {
				$data['uploadidentity_stempel'] = "Upload Gagal !<br/>" . $config['upload_path'] . $identity_stempel . $this->upload->display_errors();
			} else {
				$data['uploadidentity_stempel'] = "Upload Success !";
				$input['identity_stempel'] = $identity_stempel;
			}
		}
