...
<body class="  ">
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div class="row">
			<?php if (isset($_GET['daurulang']) && !isset($_POST['new']) && !isset($_POST['edit'])) {
				$coltitle = "col-md-8";
			} else {
				$coltitle = "col-md-10";
			} ?>
							...
							<div class="">
								...
								<form class="form-horizontal" method="post" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<div class="radio">
												<label><input value="in" type="radio" <?php if ($gudang_inout == "in") {
																							echo 'checked="checked"';
																						} ?> name="gudang_inout">In</label>
											</div>
											<div class="radio">
												<label><input value="out" type="radio" <?php if ($gudang_inout == "out") {
																							echo 'checked="checked"';
																						} ?> name="gudang_inout">Out</label>
											</div>
										</div>
									</div>
									<div class="form-group">
										...
										<tbody>
											...
											foreach ($usr->result() as $gudang) {
												...
												$price1 = $this->db
													...
													->get("invproduct");
												foreach ($price1->result() as $price1) {
													$price = $price1->invproduct_qty * $price1->invproduct_price;
												}
												if ($gudang->project_name != null) {
													$project_name = $gudang->project_name;
												} else {
													$project_name = "";
												}
												if ($gudang->product_picture != "") {
													$gambar = $gudang->product_picture;
												} else {
													$gambar = "noimage.png";
												}
											?>
												<tr>
													...
													if ($gudang->sjkeluar_no != "") {
														$customer_name = $gudang->customer_name;
													} else {
														$customer_name = "";
													}
													?>
											...
											Ekspedisi: " . $gudang->sjkeluar_ekspedisi;
													...
													<td><?= $gudang->gudang_qty;
														$tqty += $gudang->gudang_qty; ?></td>
													<?php if ($this->session->userdata("position_id") != -1) { ?>
