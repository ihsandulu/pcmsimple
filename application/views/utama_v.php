		...
		<div style="margin-bottom:30px; border-radius:5px; background-color:#FEEFC2; padding:15px;">
			...
			<form class="form-inline">
				<div class="form-group">
					...
					<input type="date" class="form-control date" id="dari" name="dari" value="<?= $dari; ?>">
				</div>
				<div class="form-group">
					<label for="pwd">To:</label>
					<input type="date" class="form-control date" id="ke" name="ke" value="<?= $ke; ?>">
				</div>
		...
		<?php if ($_SESSION['position_id'] == "1" || $_SESSION['position_id'] == "5" || $_SESSION['position_id'] == "7") { ?>
								...
								<div class="col-sm-10 col-lg-10 widget-right">
									<div class="large" style="font-size:18px;">
										...
										$i = $this->db
											...
											->where("SUBSTR(project_datetime,1,10)>=", $dari)
											->where("SUBSTR(project_datetime,1,10)<=", $ke)
											->get("project");
						...
						<div class="row no-padding">
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									<?php
									$p = $this->db
										->join("invpayment", "invpayment.invpayment_no=invpaymentproduct.invpayment_no", "")
										->where("SUBSTR(invpayment_date,1,10)>=", $dari)
										->where("SUBSTR(invpayment_date,1,10)<=", $ke)
						...
						<div class="row no-padding">
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									<?php
									$p = $this->db
										->join("invspayment", "invspayment.invspayment_no=invspaymentproduct.invspayment_no", "")
										->where("SUBSTR(invspayment_date,1,10)>=", $dari)
										->where("SUBSTR(invspayment_date,1,10)<=", $ke)
						...
						<div class="row no-padding">
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									<?php
									$usr = $this->db
										->where("SUBSTR(inv_date,1,10)>=", $dari)
										->where("SUBSTR(inv_date,1,10)<=", $ke)
										->group_by("inv_no")
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									...
									$i = $this->db
										...
										->where("SUBSTR(kas_date,1,10)>=", $dari)
										->where("SUBSTR(kas_date,1,10)<=", $ke)
										->where("kas_inout", "in")
										->group_by("kas_inout")
										->get("kas");
									if ($i->num_rows() > 0) {
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									...
									$i = $this->db
										...
										->where("SUBSTR(petty_date,1,10)>=", $dari)
										->where("SUBSTR(petty_date,1,10)<=", $ke)
						...
						<div class="row no-padding">
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									<?php
									$opnt = $this->db
										->where("SUBSTR(quotation_date,1,10)>=", $dari)
										->where("SUBSTR(quotation_date,1,10)<=", $ke)
						...
						<div class="row no-padding">
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									<?php
									$opnt = $this->db
										->where("SUBSTR(po_date,1,10)>=", $dari)
										->where("SUBSTR(po_date,1,10)<=", $ke)
										->group_by("po_no")
										->get_where("po");
									echo $opnt->num_rows();
									?>
						...
						<div class="row no-padding">
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									<?php
									$opnt = $this->db
										->where("SUBSTR(sjmasuk_date,1,10)>=", $dari)
										->where("SUBSTR(sjmasuk_date,1,10)<=", $ke)
										->get_where("sjmasuk");
									echo $opnt->num_rows();
									?>
								</div>
						...
						<div class="row no-padding">
							...
							<div class="col-sm-10 col-lg-10 widget-right">
								<div class="large" style="font-size:18px;">
									<?php
									$opnt = $this->db
										->where("SUBSTR(sjkeluar_date,1,10)>=", $dari)
										->where("SUBSTR(sjkeluar_date,1,10)<=", $ke)
										->get_where("sjkeluar");
									echo $opnt->num_rows();
									?>
								</div>
					...
					series: [{
						...
						data: [
							<?php $dipesankesupplier = $this->db
								...
								->where("SUBSTR(invs_date,1,10)>=", $dari)
								->where("SUBSTR(invs_date,1,10)<=", $ke)
								->order_by("jml", "desc")
								->limit("10")
								->get("invsproduct");
				...
				Highcharts.chart('termurah', {
					...
					series: [{
						...
						data: [<?php $dipesankesupplier = $this->db
								...
								foreach ($dipesankesupplier->result() as $a) { ?> {
									name: '<?= $a->product_name; ?>',
									y: <?= number_format($a->invsproduct_price / 1000, 0, ",", "."); ?>
								},
							<?php } ?>
						]
					}]
			...
			<script>
				Highcharts.chart('supplier', {
					...
					xAxis: {
						categories: [<?php $product = $this->db->get("product");
					...
					series: [{
						...
						data: [
							...
							<?php $dipesankesupplier = $this->db
								...
								->where("SUBSTR(gudang_datetime,1,10)>=", $dari)
								->where("SUBSTR(gudang_datetime,1,10)<=", $ke)
					...
					series: [{
						...
						data: [
							...
							<?php $dipesankesupplier = $this->db
								...
								->join("poc", "poc.poc_no=pocproduct.poc_no", "left")
								->where("SUBSTR(poc_date,1,10)>=", $dari)
								->where("SUBSTR(poc_date,1,10)<=", $ke)
					...
					series: [{
						...
						data: [
							...
							foreach ($termahal->result() as $a) { ?> {
									name: '<?= $a->product_name; ?>',
									y: <?= number_format($a->product_sell / 1000000, 0, ",", "."); ?>
								},
							<?php } ?>
					...
					xAxis: {
						categories: [
							<?php $product = $this->db
								...
								->get("invsproduct");
							foreach ($product->result() as $product) { ?> '<?= $product->product_name; ?>',
							<?php } ?>
						],
						title: {
							text: null
					...
					xAxis: {
						categories: [
							<?php $tgl = $this->db
								...
								->get("poc");
							foreach ($tgl->result() as $tgl) { ?> '<?= $tgl->poc_date; ?>',
							<?php } ?>
						]
					},
					yAxis: {
		...
		<?php } else { ?>
			<div class="well" style="color:black !important; text-shadow:#999999 1px 1px; text-align:center; padding:50px;">
				...
				<?php if ($data["user_picture"] != "") {
					$user_image = "assets/images/user_picture/" . $data["user_picture"];
				} else {
