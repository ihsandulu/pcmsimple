							...
							<div style="margin-bottom:30px !important; border:black solid 1px; border-radius:5px; padding:5px;" align="center">
								...
								<div>
									...
									if (isset($_GET['dari']) && $_GET['dari'] != "") {
										echo "From : " . date("d M Y", strtotime($_GET['dari']));
									}

									if (isset($_GET['ke']) && $_GET['ke'] != "") {
										echo "&nbsp;&nbsp;To : " . date("d M Y", strtotime($_GET['ke']));
									}
									?>
								</div>
							</div>
										...
										?>
											<tr>
												...
												<td style="text-align:right;">
													<?php
													if ($invsproduct->invspaymentproduct_amount > 0) {
														$amount = $invsproduct->invspaymentproduct_amount;
													} else {
														$amount = 0;
													}
												...
												<td style="text-align:right;">
													<?= number_format($invsproduct->invspaymentproduct_amount * $invsproduct->invspaymentproduct_qty, 0, ",", ".");
									...
									<tbody>
										...
										foreach ($usr->result() as $invproduct) { ?>
											<tr>
												...
												<td style="text-align:right;"><?= number_format($invproduct->invpaymentproduct_amount * $invproduct->invpaymentproduct_qty, 0, ",", ".");
																				$totalpenjualan += $invproduct->invpaymentproduct_amount * $invproduct->invpaymentproduct_qty; ?></td>
											</tr>
										<?php } ?>
									</tbody>
								</table>
