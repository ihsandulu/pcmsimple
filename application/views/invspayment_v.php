...
if (isset($_GET["listtable"])) { ?>
	<table id="dataTable1" class="table table-condensed table-hover">
		...
		<tbody>
			<?php
			if (!isset($_GET["tagihan"]) || $_GET["tagihan"] == "") {
				if (isset($_GET['dari'])) {
			...
			foreach ($usr->result() as $invspayment) {
				...
				if ($pr->num_rows() > 0) {
					$invsppro_amount = $pr->row()->invsppro_amount;
				}
				...
				<tr class="<?= $bmerah; ?>">
					...
					<td><?= $invspayment->methodpayment_name; ?><br />
						(<?= $invspayment->invspayment_purpose; ?>)</td>
						...
						<td>
							...
							<script>
								function inputproject(a, b) {
									$.get("<?= site_url("api/inputproject_paymentsupplier"); ?>", {
											project_id: a.value,
											invspayment_no: b
										})
										.done(function(data) {
		...
		<script>
			...
			function listtable() {
				$.ajax({
						...
						data: {
							listtable: 'OK',
							tagihan: '<?= (isset($_GET['tagihan'])) ? $this->input->get("tagihan") : ""; ?>'
						},
						dataType: "html"
					})
		...
		<script>
			function baru(invspaymentproduct_id, invspayment_no) {
				$.ajax({
						...
						method: "POST",
						data: {
							invspaymentproduct_id: invspaymentproduct_id,
							new: 'OK',
							invspayment_no: invspayment_no
						},
						dataType: "html"
			...
			function deleta(invspayment_no, invspaymentproduct_id, invspaymentproduct_source, kas_id, petty_id) {
				$.ajax({
						...
						data: {
							delete: 'OK',
							invspaymentproduct_id: invspaymentproduct_id,
							invspaymentproduct_source: invspaymentproduct_source,
							kas_id: kas_id,
							petty_id: petty_id
						},
						dataType: "html"
			...
			function cu(cu, invspaymentproduct_source, biaya_id, invspaymentproduct_description, invspaymentproduct_amount, invspaymentproduct_qty, invspaymentproduct_id, petty_id, kas_id, invspayment_no) {
				...
				$.ajax({
						...
						data: {
							...
							invspaymentproduct_source: invspaymentproduct_source,
							biaya_id: biaya_id,
							invspaymentproduct_description: invspaymentproduct_description,
							invspaymentproduct_amount: invspaymentproduct_amount,
							invspaymentproduct_qty: invspaymentproduct_qty,
							invspaymentproduct_id: invspaymentproduct_id,
							petty_id: petty_id,
							kas_id: kas_id,
							invspayment_no: invspayment_no
		...
		<script>
			function cashback(invspayment_id, invspayment_back) {
				$.ajax({
						...
						data: {
							caback: "OK",
							invspayment_id: invspayment_id,
							invspayment_back: invspayment_back
						},
						dataType: "html"
					})
					...
					<div class="panel panel-default">
						<div class="panel-body">
							<?php if (isset($_POST['new']) || isset($_POST['edit'])) { ?>
								...
								<div class="">
									<?php if (isset($_POST['edit'])) {
										...
										$judul = "Update " . $invspayment_status;
									} else {
										$namabutton = 'name="create"';
										$judul = "New " . $invspayment_status;
									} ?>
									<div class="lead">
										<h3><?= $judul; ?></h3>
									</div>
									...
									<form class="form-horizontal" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											...
											<div class="col-sm-10">
												...
												<?php } else { ?>
													<select onchange="isiuser()" id="invspayment_payto" name="invspayment_payto" class="form-control select2">
														<option value="" <?= ($invspayment_payto == "") ? "selected" : ""; ?>>
															Choose Requestor
														</option>
														<?php
														$user_query = $this->db->order_by("user_name", "ASC")->get("user");
														foreach ($user_query->result() as $user) { ?>
															<option data-user-id="<?= $user->user_id; ?>" value="<?= $user->user_name; ?>" <?= ($user->user_id == $user_id) ? "selected" : ""; ?>>
																<?= $user->user_name; ?>
															</option>
														<?php } ?>
													</select>
													...
													<script>
														function isiuser() {
															// Ambil user_id dari atribut data-user-id pada option yang dipilih
															let user_id = $("#invspayment_payto option:selected").data("user-id");

															// Setel nilai input text berdasarkan user_id yang dipilih
															$("#user_id").val(user_id);
														}
													</script>
										...
										<div class="form-group">
											...
											<div class="col-sm-10">
												...
												<?php } else { ?>
													<select name="invspayment_department" class="form-control select2">
														<option value="" <?= ($invspayment_department == "") ? "selected" : ""; ?>>
															Choose POsition
														</option>
														<?php $position = $this->db
															->order_by("position_name", "ASC")
															->get("position");
														foreach ($position->result() as $position) { ?>
															<option value="<?= $position->position_id; ?>" <?= ($position->position_id == $invspayment_department) ? "selected" : ""; ?>>
											...
											<div class="form-group">
												...
												<div class="col-sm-10">
													<select <?= $readonly; ?> autofocus class="form-control" id="invspayment_purpose" name="invspayment_purpose">
														...
														<option value="Loan" <?= ($invspayment_purpose == "Loan") ? "selected" : ""; ?>>Loan</option>
										...
										<div class="form-group">
											...
											<div class="col-sm-10">
												<input type="text" <?= $readonly; ?> autofocus class="form-control" id="invspayment_prepareby" name="invspayment_prepareby" value="<?= ($invspayment_prepareby == "") ? $this->session->userdata("user_name") : $invspayment_prepareby; ?>">
											</div>
										</div>
										<div class="form-group">
							...
							<?php } elseif (isset($_POST['settlement'])) { ?>
								<?php if ($uploadinvspaymentpicture_name != "") {
											...
											<tbody>
												...
												foreach ($usr->result() as $invspayment) {
													...
													<tr class="<?= $bmerah; ?>">
														...
														<td><?= $invspayment->invspayment_no; ?></td>
														<td>
															<?= $invspayment->methodpayment_name; ?><br />
															(<?= $invspayment->invspayment_purpose; ?>)
														</td>
											...
											<table id="" class="table table-condensed table-hover">
												...
												<tbody>
													...
													foreach ($usr->result() as $invspaymentproduct) {
														...
														if ($invspaymentproduct->invspaymentproduct_source == "petty_id") {
															$source = "Petty Cash";
														} else {
															$source = "Big Cash";
														}
